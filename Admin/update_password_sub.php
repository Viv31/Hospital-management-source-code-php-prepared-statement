...
/*include_once('conn/config.php');
 ...
 $new_password = md5($_POST['new_password']);
  ...
  include_once("conn/config.php");
      ...
      $chk_pass = "SELECT `id`, `admin_password` FROM `admin` WHERE admin_password = '".$user_password."' AND id = '".$user_id."'";
      $fire = mysqli_query($conn,$chk_pass);
      //die;
      if(mysqli_num_rows($fire)>0){
        
        $update_user_password = "UPDATE `admin` SET `admin_password` = '".$new_password."' WHERE id = '".$user_id."'";
